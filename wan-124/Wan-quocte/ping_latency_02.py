...
def check_host(ip_dest):
    for hostname in ip_dest:
        ...
        if response == 0:
            ...
            telegram_net(status)    ## check host error
...
def check_loss(ip_dest):
    ...
    if int(loss_01[0]) >= 10 and int(loss_02[0]) >= 10 and \
       int(loss_03[0]) >= 10:
        message = "WAN-124-HTC -QuocTe- packet loss 3 host đồng thời: "\
...
def check_latency(ip_dest):
    ...
    if int(float(ping_01['avg'])) >= 75 and int(float(ping_02['avg'])) >= 75 and\
       int(float(ping_03['avg'])) >= 75:
        message = "WAN-124-HTC -QuocTe- Latency 3 host đồng thời Cao > 75ms: " +\
                  str(ip_dest[0]) + " ," + str(ip_dest[1]) + " ," +\
                  str(ip_dest[2])
