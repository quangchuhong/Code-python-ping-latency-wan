...
def check_loss(ip_dest):
    ...
    if int(loss_01[0]) >= 10 and int(loss_02[0]) >= 10 and \
       int(loss_03[0]) >= 10:
        message = "WAN-111-VNPT -VN- packet loss 3 host đồng thời: "\
...
def check_latency(ip_dest):
    ...
    ping_02 = ping(ip_dest[1])
    ping_03 = ping(ip_dest[2])
    if int(float(ping_01['avg'])) >= 10 and int(float(ping_02['avg'])) >= 10 and\
       int(float(ping_03['avg'])) >= 10:
        message = "WAN-111-VNPT -VN- Latency 3 host đồng thời Cao > 10ms: " +\
                  str(ip_dest[0]) + " ," + str(ip_dest[1]) + " ," +\
                  str(ip_dest[2])
        ketqua = "Latency Cao tren 10ms: " + str(ip_dest[0]) + ": " +\
...
while True:
    ip_dest=['dantri.com.vn','vtv.vn','vietnamnet.vn']
    #check_host(ip_dest)
