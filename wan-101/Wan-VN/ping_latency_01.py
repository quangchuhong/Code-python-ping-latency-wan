...
def check_loss(ip_dest):
    ...
    if int(loss_01[0]) >= 10 and int(loss_02[0]) >= 10 and \
       int(loss_03[0]) >= 10:
        message = "WAN-101-Viettel -VN- packet loss 3 host đồng thời: "\
...
def check_latency(ip_dest):
    ...
    ping_02 = ping(ip_dest[1])
    ping_03 = ping(ip_dest[2])
    if int(float(ping_01['avg'])) >= 65 and int(float(ping_02['avg'])) >= 65 and\
       int(float(ping_03['avg'])) >= 65:
        message = "WAN-101-Viettel -VN- Latency 3 host đồng thời Cao > 50ms: " +\
                  str(ip_dest[0]) + " ," + str(ip_dest[1]) + " ," +\
                  str(ip_dest[2])
...
while True:
    ip_dest=['baomoi.com','vov.vn','lazada.vn']
    #check_host(ip_dest)
