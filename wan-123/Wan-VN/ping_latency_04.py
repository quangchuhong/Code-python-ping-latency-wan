...
def check_latency(ip_dest):
    ...
    ping_02 = ping(ip_dest[1])
    ping_03 = ping(ip_dest[2])
    if int(float(ping_01['avg'])) >= 60 and int(float(ping_02['avg'])) >= 60 and\
       int(float(ping_03['avg'])) >= 60:
        message = "WAN-123-VNPT -VN- Latency 3 host đồng thời Cao > 50ms: " +\
                  str(ip_dest[0]) + " ," + str(ip_dest[1]) + " ," +\
                  str(ip_dest[2])
...
while True:
    ip_dest=['thanhnien.vn','phimmoi.net','zing.vn']
    #check_host(ip_dest)
