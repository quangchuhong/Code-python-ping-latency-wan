...
import os, time
import subprocess
from telegram import telegram_packet_loss, telegram_lan,\
                     telegram_wan_quocte, telegram_wan_vccorp,\
                     telegram_wan_vietnam, telegram_network_error


def ping(ip_dest):

    cmd = "ping -I 192.168.17.222 -c 4 {}".format(ip_dest).split(' ')
    try:
...
def check_loss(ip_dest):
    ...
    if int(loss_01[0]) >= 10 and int(loss_02[0]) >= 10 and \
       int(loss_03[0]) >= 10:
        message = "WAN-123-VNPT -VCCORP- packet loss 3 host đồng thời: "\
...
def check_latency(ip_dest):
    ...
    ping_02 = ping(ip_dest[1])
    ping_03 = ping(ip_dest[2])
    if int(float(ping_01['avg'])) >= 30 and int(float(ping_02['avg'])) >= 30 and\
       int(float(ping_03['avg'])) >= 30:
        message = "WAN-123-VNPT -VCCORP- Latency 3 host đồng thời Cao > 30ms: " +\
                  str(ip_dest[0]) + " ," + str(ip_dest[1]) + " ," +\
                  str(ip_dest[2])
        ketqua = "Latency Cao tren 30ms: " + str(ip_dest[0]) + ": " +\
                 str(ping_01['avg']) + "ms---" + str(ip_dest[1]) + ": " +\
                 str(ping_02['avg']) + "ms---" + str(ip_dest[2]) + ": " +\
                 str(ping_03['avg'])+"ms"
        telegram_wan_vccorp(message)
        telegram_wan_vccorp(ketqua)
        # traceroute domain
        tracer_domain(ip_dest)
    else:
        print "Network Latency OK!"
while True:
    ip_dest=['afamily.vn','cafebiz.vn','gamek.vn']
    #check_host(ip_dest)
