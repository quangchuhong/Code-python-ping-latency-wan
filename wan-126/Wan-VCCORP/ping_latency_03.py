...
def ping(ip_dest):
    ...
    cmd = "ping -I 192.168.17.225 -c 4 {}".format(ip_dest).split(' ')
    try:
...
def check_loss(ip_dest):
    ...
    if int(loss_01[0]) >= 10 and int(loss_02[0]) >= 10 and \
       int(loss_03[0]) >= 10:
        message = "WAN-126-FPT -VCCORP- packet loss 3 host đồng thời: "\
...
def check_latency(ip_dest):
    ...
    if int(float(ping_01['avg'])) >= 30 and int(float(ping_02['avg'])) >= 30 and\
       int(float(ping_03['avg'])) >= 30:
        message = "WAN-126-FPT -VCCORP- Latency 3 host đồng thời Cao > 30ms: " +\
                  str(ip_dest[0]) + " ," + str(ip_dest[1]) + " ," +\
                  str(ip_dest[2])
...
while True:
    ip_dest=['vccorp.vn','tuoitre.vn','kenh14.vn']
    #check_host(ip_dest)
